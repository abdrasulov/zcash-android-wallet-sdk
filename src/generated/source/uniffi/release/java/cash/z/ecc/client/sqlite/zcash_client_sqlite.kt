...
package cash.z.ecc.client.sqlite
...
open class RustBuffer : Structure() {
    ...
    companion object {
        internal fun alloc(size: Int = 0) = rustCall() { status ->
            _UniFFILib.INSTANCE.ffi_zcash_client_sqlite_e975_rustbuffer_alloc(size, status)
        }

        internal fun free(buf: RustBuffer.ByValue) = rustCall() { status ->
            _UniFFILib.INSTANCE.ffi_zcash_client_sqlite_e975_rustbuffer_free(buf, status)
        }

        internal fun reserve(buf: RustBuffer.ByValue, additional: Int) = rustCall() { status ->
            _UniFFILib.INSTANCE.ffi_zcash_client_sqlite_e975_rustbuffer_reserve(buf, additional, status)
        }
    }
...
internal interface _UniFFILib : Library {
    companion object {
        internal val INSTANCE: _UniFFILib by lazy {
            loadIndirect<_UniFFILib>(componentName = "zcash_client_sqlite")
        }
    }

    fun zcash_client_sqlite_e975_derive_spending_keys(
    ...
    fun zcash_client_sqlite_e975_derive_extfvk(
        network: RustBuffer.ByValue,
        extsk: RustBuffer.ByValue,
        uniffi_out_err: RustCallStatus
    ): RustBuffer.ByValue

    fun ffi_zcash_client_sqlite_e975_rustbuffer_alloc(
        size: Int,
        uniffi_out_err: RustCallStatus
    ): RustBuffer.ByValue

    fun ffi_zcash_client_sqlite_e975_rustbuffer_from_bytes(
        bytes: ForeignBytes.ByValue,
        uniffi_out_err: RustCallStatus
    ): RustBuffer.ByValue

    fun ffi_zcash_client_sqlite_e975_rustbuffer_free(
        buf: RustBuffer.ByValue,
        uniffi_out_err: RustCallStatus
    ): Unit

    fun ffi_zcash_client_sqlite_e975_rustbuffer_reserve(
        buf: RustBuffer.ByValue,
        additional: Int,
        uniffi_out_err: RustCallStatus
    ): RustBuffer.ByValue
...
enum class Network {
    ...
    companion object {
        ...
        internal fun read(buf: ByteBuffer) =
            try { values()[buf.getInt() - 1] } catch (e: IndexOutOfBoundsException) {
    ...
    companion object ErrorHandler : CallStatusErrorHandler<Exception> {
        ...
        fun read(error_buf: ByteBuffer): Exception {
            return when (error_buf.getInt()) {
                1 -> Exception.InvalidExtSk()
                2 -> Exception.WrongNetwork()
...
@ExperimentalUnsignedTypes
fun deriveSpendingKeys(seed: List<UByte>, network: Network, accounts: UInt): List<String> {
    val _retval =
        rustCall() { status ->
            _UniFFILib.INSTANCE.zcash_client_sqlite_e975_derive_spending_keys(lowerSequenceu8(seed), network.lower(), accounts.lower(), status)
        }
    return liftSequencestring(_retval)
}

fun deriveExtfvk(network: Network, extsk: String): String {
    val _retval =
        rustCallWithError(Exception) { status ->
            _UniFFILib.INSTANCE.zcash_client_sqlite_e975_derive_extfvk(network.lower(), extsk.lower(), status)
        }
